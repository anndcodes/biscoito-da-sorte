...
let randomMessage = Math.round(Math.random() * messages.length);

// Events
closedCookie.addEventListener("click", handleOpenCookie);
btnOpenCookie.addEventListener("click", handleOpenAnotherCookie);
...
function handleOpenAnotherCookie() {
  randomMessage = Math.round(Math.random() * messages.length);
  toggleCards();
}
